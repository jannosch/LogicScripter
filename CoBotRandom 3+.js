...
// Field for Sensor-Notes
let sliders = [];

// Stored played Note to Mute it later
let playedNote;

let pSensorNote = {name: "Sensor-Note", type: "note"};
// Field for Stop-Notes
let pStopNote = {name: "Stop-Note", type: "note"};

let pStopMode = {name: "Stop-Modus", type: "menu", defaultValue: 0, valueStrings: ["Stoppe nur bei Stop-Note", "Stoppe wenn Sensor-Note losgelassen"], sub: true};

let pMode = {name: "Modus", type: "menu", defaultValue: 0, valueStrings: ["Zufall", "Zufallssequenz (geht noch nicht)"], sub: true, subParams:[[], [
        {name: "Wiederhole nach", type: "lin", defaultValue: 16, minValue: 0, maxValue: 64, numberOfSteps: 64, unit: "Beats", sub: true},
        {name: "Sequenz zurücksetzen", type: "momentary", sub: true}
    ]]};
let pPassThrough = {name: "MIDI-Filter", type: "menu", defaultValue: 2, valueStrings: ["Nichts filtern", "Trigger-Noten filtern", "Alle Noten filtern", "Alles filtern"], sub: true};
let pExtendedOptions = {name: "Mehr Optionen", type: "momentary", subParams:[
        pStopMode,
        pMode,
        pPassThrough
    ]};
let pRandomNotes = {name: "random", type: "expand", params: [
        {name: "%. Zufallston", type: "note"},
        {name: "%. Warscheinlichkeit", type: "lin", defaultValue: 0, minValue: 0, maxValue: 100, numberOfSteps: 100000, unit: "%", listener: function (value) {adjustSliders(this, value);}}
    ]};

// Called once on creation
function setup() {
    addParameter(pSensorNote);
    addParameter(pStopNote);
    addParameter(pExtendedOptions);
    addParameter(pRandomNotes);
}
...
function handleMIDI(event) {
    ...
    if (event instanceof NoteOn) {
        ...
        // If Sensor-Note is triggered
        if (event.pitch === getNoteParameter(pSensorNote)) {

            // Stop played note
            if (playedNote !== undefined) {
                sendNoteOff(playedNote, event.velocity);
                playedNote = undefined;
            }

            let random = Math.random() * 100;  // Random Number 0-99.9999
            let probCounter = 0;               // Count probability sliders together
            for (let i = 0; i < sliders.length; i++) {
                probCounter += getParameter(sliders[i]);

                // If this parameter is selected
                if (probCounter > random) {
                    let noteParameter = getCopies(pRandomNotes, 0)[i];
                    let pitch = getNoteParameter(noteParameter);

                    // If valid Note is selected
                    if (pitch >= 0) {

                        // Play new note
                        sendNoteOn(pitch, event.velocity);
                        playedNote = pitch;
                        break;
                    }
                }
            }
            // Play Original Note if wanted
            if (getParameter(pPassThrough) === 0) event.send();


        // If Mute-Note is triggered
        } else if (event.pitch === getNoteParameter(pStopNote)) {
            if (playedNote !== undefined) {
                // Stop played note
                sendNoteOff(playedNote, event.velocity);
                playedNote = undefined;
        ...
        // Pass every non-Trigger Note if selected
        } else if (getParameter(pPassThrough) <= 1) event.send();



    } else if (event instanceof NoteOff) {
        // If Sensor-Note is released and stop by release is selected
        if (event.pitch === getNoteParameter(pSensorNote)) {
            if (getParameter(pStopMode) === 1) {
                if (playedNote !== undefined) {
                    sendNoteOff(playedNote, event.velocity);
                    playedNote = undefined;
                }
        ...
        if (event.pitch === getNoteParameter(pStopNote)) {
            if (getParameter(pPassThrough) === 0) event.send();
            return;
        }


        if (getParameter(pPassThrough) <= 1) event.send();


    } else {
        if (getParameter(pPassThrough) <= 2) event.send();
...
function adjustSliders(paramObject, value) {
    let clones = getCopies(paramObject, 1);
    let totalValue = 0;

    // Copy clones to SliderArray needed for Note-Randomization
    sliders = [...clones];

    // Delete slider from Array
    clones.splice(clones.indexOf(paramObject), 1);

    // Count all percentages together
    for (let i = 0; i < clones.length; i++) {
        totalValue += GetParameter(PluginParameters.indexOf(clones[i]));
    }

    // If above 100 reduce to 100
    if (totalValue + value > 100) {

        // Factor to multiply every other Tone
        let factor = (100 - value) / totalValue;

        for (let i = 0; i < clones.length; i++) {
            let paramIndex = PluginParameters.indexOf(clones[i]);
            SetParameter(paramIndex, GetParameter(paramIndex) * factor);
        }
    }
}
 ...
 *  index is necessary if the Expanded object has multible childs
 */
